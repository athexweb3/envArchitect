...
impl Plugin for PythonPlugin {
    async fn resolve(&self, _context: &ResolutionContext) -> Result<(InstallPlan, Option<String>)> {
        ...
        plan.manifest.env.insert("python".to_string(), version_req.clone());

        // 3. Determine Installation Strategy
        // Recommendation: use a manageable python
        plan.instructions.push(format!("echo 'Python {} required.'", version_req));
        
        if host::read_file("requirements.txt").is_ok() {
            plan.instructions.push("pip install -r requirements.txt".to_string());
